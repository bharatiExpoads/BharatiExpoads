...
const ContraVoucherMaster = () => {
  ...
  const fetchVouchers = async () => {
    ...
    setError(null);
    try {
      const data = await masterService.getAllContraVouchers();
      setVouchers(data);
    } catch (err) {
      setError(err.message || 'Failed to fetch contra vouchers');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedVoucher) {
        await masterService.updateContraVoucher(selectedVoucher.id, cleanData);
      } else {
        await masterService.createContraVoucher(cleanData);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      await masterService.deleteContraVoucher(selectedVoucher.id);
      fetchVouchers();
      setIsDeleteModalOpen(false);
    } catch (err) {
      setError(err.message || 'Failed to delete contra voucher');
    }
  };
    ...
    <Layout>
      <div className="container mx-auto py-6">
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-2xl font-bold">Contra Voucher Master</h2>
        </div>
        ...
        <Modal
          ...
          title={selectedVoucher ? 'Edit Contra Voucher' : 'Add Contra Voucher'}
        >
...
export default ContraVoucherMaster;
